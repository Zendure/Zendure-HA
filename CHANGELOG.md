...
- zenSDK MQTT push: integration subscribes to `Zendure/+/{SN}/#` and processes per-property updates in real time — no more 60s HTTP polling when MQTT is connected
- zenSDK write commands now publish to `Zendure/{type}/{SN}/{property}/set` via MQTT instead of HTTP POST when MQTT is active
- MQTT: configure local MQTT directly from the integration — no separate curl commands required
