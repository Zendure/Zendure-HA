...
def distribute_powerrrr(devices: List[Any], power_to_devide: int, main_state: MainState) -> Dict[Any, int]:
    ...
    # 1. Kandidaten nach SOC
    if main_state == MainState.GRID_CHARGE:
        candidates = [d for d in devices if d.state != DeviceState.SOCFULL]
    else:  # DISCHARGE
        candidates = [d for d in devices if d.state != DeviceState.SOCEMPTY]

    if not candidates:
        return {d: 0 for d in devices}

    # 2. Mit letztem Count starten
    active_count = min(max(1, _last_active_count), len(candidates))

    # 3. Last % bestimmen (nur 1 Gerät reicht, da proportional verteilt wird)
    active_devs = candidates[:active_count]
    first = active_devs[0]
    ...
    if load_pct > 60 and active_count < len(candidates):
        ...
        #hier das letzte gerät aus candidates[:active_count] auf 0 power setzten
        active_count -= 1
        _LOGGER.info("minus count")

    _LOGGER.info(f"last count {_last_active_count} activecount {active_count}")
    _last_active_count = active_count

    # Nur das erste Kandidat-Device prüfen und rotieren
    ROTATION_THRESHOLD = 0.001  # 20 %
    if candidates:
        first = candidates[0]
        if abs(first.energy_diff_kwh) >= ROTATION_THRESHOLD * first.kWh:
            _LOGGER.info(
                f"Rotation: {first.name} hat {first.energy_diff_kwh:.2f} kWh erreicht "
                f"({ROTATION_THRESHOLD*100:.0f}% von {first.kWh} kWh)."
            )
            # Reset Counter
            first.energy_diff_kwh = 0

            # Gerät nach hinten rotieren
            candidates.append(candidates.pop(0))
            _LOGGER.info(f"{first.name} wurde nach hinten verschoben.")

    # 5. Aktive Geräte festlegen
    active_devs = candidates[:active_count]
    allocation: Dict[Any, int] = {}
    ...
    if main_state == MainState.GRID_DISCHARGE:
        ...
        if pv_sum_active > 0 and abs(power_to_devide) >= 0.9 * pv_sum_active:
            for d in candidates:
                if d not in active_devs and d.pwr_solar > 50:
                    _LOGGER.info(f"Aktiviere zusätzlich {d.name} wegen PV={d.pwr_solar}W")
                    active_devs.append(d)

        pv_sum_active = sum(d.pwr_solar for d in active_devs)
        
        if abs(power_to_devide) <= pv_sum_active:
            total_pv = sum(d.pwr_solar for d in active_devs if d.pwr_solar > 50)
            for d in active_devs:
                share = (d.pwr_solar / total_pv) if total_pv > 0 else 0
                allocation[d] = int(abs(power_to_devide) * share)
        else:
            # 1. PV voll nutzen (begrenzt durch Device-Limit)
            for d in active_devs:
                allocation[d] = min(d.pwr_solar, d.limitDischarge)

            # 2. Rest aus Batterie proportional verteilen
            rest = abs(power_to_devide) - pv_sum_active
            total_limit = sum(d.limitDischarge for d in active_devs)
            for d in active_devs:
                allocation[d] += int(rest * d.limitDischarge / total_limit)

    else:  # GRID_CHARGE
        total_limit = sum(abs(d.limitCharge) for d in active_devs)
        for d in active_devs:
            allocation[d] = int(abs(power_to_devide) * abs(d.limitCharge) / total_limit)

    return allocation



def distribute_powerr(devices: List[Any], power_to_devide: int, pwr_solar: int, main_state: MainState) -> Dict[Any, int]:
    global _last_active_count
    ...
    # 3. Last % bestimmen
    per_dev_power = abs(power_to_devide) / active_count
    max_power = max(
        d.limitDischarge if main_state == MainState.GRID_DISCHARGE else abs(d.limitCharge)
        for d in candidates
    )
    load_pct = (per_dev_power / max_power) * 100 if max_power > 0 else 0
    ...
    if main_state == MainState.GRID_DISCHARGE:
        total_limit = sum(d.limitDischarge for d in active_devs)
        for d in active_devs:
            allocation[d] = int(abs(power_to_devide) * d.limitDischarge / total_limit)
    else:  # CHARGE
        total_limit = sum(abs(d.limitCharge) for d in active_devs)
        for d in active_devs:
            allocation[d] = int(abs(power_to_devide) * abs(d.limitCharge) / total_limit)

    # 6. Alle anderen Geräte explizit auf 0 setzen
    for d in devices:
