...
class ZendureManager(DataUpdateCoordinator[None], EntityDevice):
    ...
    async def update_operation(self, entity: ZendureSelect, _operation: Any) -> None:
        ...
        async with self._control_lock:
            ...
            if (
                self.p1meterEvent is not None
                and operation != ManagerMode.OFF
                and (len(self.devices) == 0 or all(not d.online for d in self.devices))
            ):
    ...
    async def _execute_control_update(
        self,
        p1: int,
        isFast: bool,
        time: datetime,
